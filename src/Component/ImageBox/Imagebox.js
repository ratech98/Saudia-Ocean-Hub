...
const Imagebox = ({numBorder,numWidth,numHeight,showPlaceName}) => {
  const imageBox = [
    {
      id: 1,
      image: Boat,
  ...
  return (
    <div>
      ...
      >
        {imageBox.map((item) => (
          <Card style={{ borderRadius: `${numBorder}px`, width: `${numWidth}` }} key={item.id}>
            <Card.Img
              variant="top"
              src={item.image}
              style={{ borderTopLeftRadius:`${numBorder}`,borderTopLeftRadius:`${numBorder}` , width:`${numWidth}px` ,height:`${numHeight}px`}}
            />
            <Card.Body>
